...
function Three() {
    ...
    const [planeta_nume, set_planeta_nume] = useState("");
    const [planeta_ind, set_planeta_ind] = useState(0);
    ...
    useEffect(() => {
        ...
        update_json_api().then(() => {
            ...
            if (!rendererRef.current) {
                ...
                camera.position.set(0, 10, 150000000);
                ...
                function animate() {
                    const date = new Date();
                    var time = date.getSeconds() + date.getMinutes() * 60;
        ...
        <div className="navbar absolute top-0 bg-transparent">
            <div className="navbar-start">
                <ul className="menu menu-horizontal px-1">
                    <input className='input w-full max-w-xs' type='number' max={8} min={1} onChange={(event) => { lookAt(event.target.value - 1); set_planeta_nume(planete[event.target.value - 1].name); set_planeta_ind(event.target.value - 1); }} />

                </ul>
            </div>
            <div className="navbar-center hidden lg:flex">
                <a className="btn btn-ghost text-xl text-white">{planeta_nume}</a>

            </div>
            <div className="navbar-end">
                <a className="btn" onClick={() => alert(info[planeta_ind])}>
                    Information
                </a>
            </div>
        </div>
